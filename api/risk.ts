import type { Indicator, RiskData } from '../types';
...
const buildFlatHistory = (val: number) =>
...
const fetchJsonWithProxy = async (url: string): Promise<any> => {
  try {
...
const buildFallbackData = (note: string): RiskData => {
  const indicators: Indicator[] = [
    {
      id: 'vix', name: 'VIX (Fear Index)', category: 'Volatility', value: 16.2, unit: '',
      direction: 'higher_is_risky', description: 'Market expectation of 30-day forward volatility.',
      weight: 0.25, history: buildFlatHistory(16.2), subScore: 20, explanation: 'Fallback data when live metrics are unavailable.'
    },
    {
      id: 'yield-curve', name: '10Y-2Y Yield Spread', category: 'Macro', value: 0.2, unit: '%',
      direction: 'lower_is_risky', description: 'Difference between long and short term yields.',
      weight: 0.15, history: buildFlatHistory(0.2), subScore: 50, explanation: 'Fallback data when live metrics are unavailable.'
    },
    {
      id: 'hy-spread', name: 'High Yield OAS', category: 'Credit/Stress', value: 3.5, unit: '%',
      direction: 'higher_is_risky', description: 'Risk premium on "junk" bonds.',
      weight: 0.2, history: buildFlatHistory(3.5), subScore: 30, explanation: 'Fallback data when live metrics are unavailable.'
    },
    {
      id: 'pe-ratio', name: 'S&P 500 P/E Ratio', category: 'Valuation', value: 20.8, unit: 'x',
      direction: 'higher_is_risky', description: 'Price relative to trailing 12m earnings.',
      weight: 0.2, history: buildFlatHistory(20.8), subScore: 55, explanation: 'Fallback data when live metrics are unavailable.'
    },
    {
      id: 'put-call', name: 'Put/Call Ratio', category: 'Volatility', value: 0.9, unit: '',
      direction: 'higher_is_risky', description: 'Options volume ratio.',
      weight: 0.1, history: buildFlatHistory(0.9), subScore: 45, explanation: 'Fallback data when live metrics are unavailable.'
    },
    {
      id: 'ted-spread', name: 'TED Spread', category: 'Liquidity', value: 0.25, unit: '%',
      direction: 'higher_is_risky', description: 'Interbank credit risk measure.',
      weight: 0.1, history: buildFlatHistory(0.25), subScore: 35, explanation: 'Fallback data when live metrics are unavailable.'
    }
  ];

  return {
    ...buildRiskPayload(indicators, []),
    sources: [{ title: note, uri: 'https://fred.stlouisfed.org/' }]
  };
};
...
const fetchRiskData = async (): Promise<RiskData> => {
  ...
  const results = await Promise.all(
    seriesConfigs.map(async (config) => {
      ...
      return {
        config,
        ...result,
        sources: result.failed ? [] : [{ title: `FRED: ${config.label}`, uri: `https://fred.stlouisfed.org/series/${config.seriesId}` }]
      };
    })
  );
  ...
  if (sources.length === 0) {
    return buildFallbackData('Live APIs are currently unavailable.');
  }
  ...
  const indicators: Indicator[] = [
    {
      id: 'vix', name: 'VIX (Fear Index)', category: 'Volatility', value: values.vix, unit: '', direction: 'higher_is_risky',
      description: 'Market expectation of 30-day forward volatility.', weight: 0.25,
    ...
    {
      id: 'yield-curve', name: '10Y-2Y Yield Spread', category: 'Macro', value: values.t10y2y, unit: '%', direction: 'lower_is_risky',
      description: 'Difference between long and short term yields.', weight: 0.15,
    ...
    {
      id: 'hy-spread', name: 'High Yield OAS', category: 'Credit/Stress', value: values.hy_oas, unit: '%', direction: 'higher_is_risky',
      description: 'Risk premium on "junk" bonds.', weight: 0.2,
    ...
    {
      id: 'pe-ratio', name: 'S&P 500 P/E Ratio', category: 'Valuation', value: values.pe, unit: 'x', direction: 'higher_is_risky',
      description: 'Price relative to trailing 12m earnings.', weight: 0.2,
    ...
    {
      id: 'put-call', name: 'Put/Call Ratio', category: 'Volatility', value: values.pc, unit: '', direction: 'higher_is_risky',
      description: 'Options volume ratio.', weight: 0.1,
    ...
    {
      id: 'ted-spread', name: 'TED Spread', category: 'Liquidity', value: values.ted, unit: '%', direction: 'higher_is_risky',
      description: 'Interbank credit risk measure.', weight: 0.1,
...
export default async function handler(_req: any, res: any) {
  try {
    const payload = await fetchRiskData();
